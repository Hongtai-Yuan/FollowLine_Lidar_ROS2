...
import threading
import math
from Follow_line_withlidar_ros2.follow_common import *
RAD2DEG = 180 / math.pi
print ("import finish")
...
class LineDetect(Node):
	def __init__(self,name):
		...
		self.sub_scan = self.create_subscription(LaserScan,"/scan",self.registerScan,5)
		self.declare_param()
		...
		self.Mouse_XY = (0, 0)
		self.hsv_text = "LineFollowHSV.text"
	...
	def execute(self, point_x, color_radius):
		...
		if color_radius == 0: 
			...
			twist = Twist()
			b = Bool()
			[z_Pid, _] = self.PID_controller.update([(point_x - 320)*1.0/36, 0])
