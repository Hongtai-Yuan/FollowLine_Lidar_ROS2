...
class color_follow:
    ...
    def Roi_hsv(self, img, Roi):
        ...
        roi_region = HSV[Roi[1]:Roi[3], Roi[0]:Roi[2]]
        h_hist = cv.calcHist([roi_region], [0], None, [256], [0, 256])
        s_hist = cv.calcHist([roi_region], [1], None, [256], [0, 256])
        v_hist = cv.calcHist([roi_region], [2], None, [256], [0, 256])
        h_cdf = np.cumsum(h_hist) / np.sum(h_hist)
        s_cdf = np.cumsum(s_hist) / np.sum(s_hist)
        v_cdf = np.cumsum(v_hist) / np.sum(v_hist)
        percentile = 0.10
        H_min = np.argmax(h_cdf >= percentile)
        H_max = np.argmax(h_cdf >= (1 - percentile))
        S_min = np.argmax(s_cdf >= percentile)
        S_max = np.argmax(s_cdf >= (1 - percentile))
        V_min = np.argmax(v_cdf >= percentile)
        V_max = np.argmax(v_cdf >= (1 - percentile))
        #H_min = np.argmax(h_hist > 0)
        #H_max = np.argmax(h_hist[::-1] > 0)
        #H_max = 255 - H_max
        #S_min = np.argmax(s_hist > 0)
        #S_max = 255 - np.argmax(s_hist[::-1] > 0)
        #V_min = np.argmax(v_hist > 0)
        #V_max = 255 - np.argmax(v_hist[::-1] > 0)
